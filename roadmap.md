# 路線圖

X World Games 將會在未來數年內完成一項的龐大平台基礎架構計劃，包括遊戲聚合中心，NFT交易所，DeFi交易機制，跨遊戲協議端口和全功能遊戲創建工具箱。同時，我們將通過與外部合作夥伴或內部開發來發布更多款遊戲，並簽署更多正版動漫IP來豐富我們的遊戲生態系統和社區，將幫助這些移植遊戲與我們的憑證，礦池和經濟系統連接起來，並指引我們的社區邁向一個全面的去中心化治理。

| 日期       | 目標                                  | 狀態            |
| -------- | ----------------------------------- | ------------- |
| Q1 2019  | 整體項目概念                              | ~~Completed~~ |
| Q2 2019  | 團隊組建                                | ~~Completed~~ |
| Q3 2019  | 第一批遊戲角色IP                           | ~~Confirmed~~ |
|          | 遊戲開發                                | ~~Started~~   |
| Q4 2019  | X World Games: 平台技術架構               | ~~Completed~~ |
| Q1 2020  | 主要遊戲框架開發                            | ~~Completed~~ |
| ~~~~     | 第一批遊戲角色IP創建                         | ~~Completed~~ |
| Q2 2020  | NFT 合约开发                            | ~~Started~~   |
| ~~~~     | 钱包访问功能开发                            | ~~Started~~   |
| Q3 2020  | 官网&交易所开发                            | ~~Started~~   |
| Q4 2020  | 游戏内战斗功能                             | ~~Completed~~ |
|          | 区块链钱包访问功能测试                         | ~~Completed~~ |
| Q1 2021  | NFT 合约安全审计                          | ~~Passed~~    |
| Q2 2021  | 官网&交易所                              | ~~Launched~~  |
| ~~~~     | Dream Card - 收集，创建，交易所，战斗功能         | ~~Completed~~ |
| Q3 2021  | X World Games - 世界社交平台以及社群建立        | ~~Launched~~  |
|          | Dream Card - 第二批游戏角色IP签署            | ~~Signed~~    |
| ~~~~     | X World Games - XWG 钱包              | ~~Released~~  |
|          | Dream Card - 远征系统                   | ~~Released~~  |
| ~~~~     | Dream Card - ELO 系统                 | ~~Released~~  |
|          | Dream Card 2.0 Demo                 | ~~Released~~  |
|          | X World Games - 100万用户达成            | ~~Completed~~ |
|          | Marketplace 2.0                     | ~~Completed~~ |
| Q4 2021  | Dream Card - 第二批游戏角色IP创建            |               |
|          | Dream Card - 2.0 Alpha              |               |
|          | X World Games - 生态游戏开发工具            |               |
|          | Marketplace - 增加池系统                 |               |
|          | X World Games - 头像设计系统              |               |
|          | DAO - 去中心化自治社群成立                    |               |
| Q1 2022  | 云3D渲染系统项目                           |               |
| Q2 2022  | 5 - 10 世界知名品牌合作IP游戏签署               |               |
| Q3 2022  | 雲3D渲染系統DEMO                         |               |
|          | X 元宇宙系统 DEMO (VR设备访问 X World Games) |               |
| Q4 2022  | 元宇宙系统 Alpha - 开放玩家使用VR登入X元宇宙        |               |
